...
class SmartFilterBot:
    ...
    def reset_state(self):
        self.state = {
            'mode': 'greeting',  # greeting -> filter_selection -> time_filtering -> awaiting_start -> awaiting_end
    ...
    def handle_filter_selection(self, user_input):
        ...
        if any(word in user_input.lower() for word in ['time', 'hour', 'shift', 'schedule']):
            ...
            self.add_message("Employee name filtering is coming soon! 👤\n\nFor now, I can help you with time-based filtering. Would you like to try that?",
                           show_options=True,
                           options=[("🕐 Yes, filter by time", "I want to filter by time")])
    ...
    def handle_awaiting_start(self, user_input):
        ...
        self.filter_and_respond(self.state['start_time'], self.state['end_time'], self.state['time_filter_type'])
